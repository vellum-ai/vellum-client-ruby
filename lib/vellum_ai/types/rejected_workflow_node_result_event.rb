...
# An event that indicates that the node has rejected its execution.
  ...
  # @return [Object] 
    ...
    def initialize(id:, node_id:, node_result_id:, ts: OMIT, data: OMIT, source_execution_id: OMIT, error:, additional_properties: nil)
      @id = id
      @node_id = node_id
      ...
      @_field_set = { "id": id, "node_id": node_id, "node_result_id": node_result_id, "ts": ts, "data": data, "source_execution_id": source_execution_id, "error": error }.reject do | _k, v |
  v == OMIT
end
    end
