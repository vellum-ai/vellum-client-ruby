...
# An event that indicates that the node has execution is in progress.
  ...
  # @return [Object] 
    ...
    def initialize(id:, node_id:, node_result_id:, ts: OMIT, data: OMIT, source_execution_id: OMIT, output: OMIT, output_index: OMIT, additional_properties: nil)
      @id = id
      @node_id = node_id
      ...
      @_field_set = { "id": id, "node_id": node_id, "node_result_id": node_result_id, "ts": ts, "data": data, "source_execution_id": source_execution_id, "output": output, "output_index": output_index }.reject do | _k, v |
  v == OMIT
end
    end
