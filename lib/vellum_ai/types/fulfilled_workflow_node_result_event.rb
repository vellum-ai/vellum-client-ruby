...
# An event that indicates that the node has fulfilled its execution.
  ...
  # @return [Object] 
    ...
    def initialize(id:, node_id:, node_result_id:, ts: OMIT, data: OMIT, source_execution_id: OMIT, output_values:, mocked: OMIT, additional_properties: nil)
      @id = id
      @node_id = node_id
      ...
      @_field_set = { "id": id, "node_id": node_id, "node_result_id": node_result_id, "ts": ts, "data": data, "source_execution_id": source_execution_id, "output_values": output_values, "mocked": mocked }.reject do | _k, v |
  v == OMIT
end
    end
