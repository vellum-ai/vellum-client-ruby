...
# An event that indicates that the node has initiated its execution.
  ...
  # @return [Object] 
    ...
    def initialize(id:, node_id:, node_result_id:, state:, ts: OMIT, data: OMIT, source_execution_id: OMIT, input_values: OMIT, additional_properties: nil)
      @id = id
      @node_id = node_id
      ...
      @_field_set = { "id": id, "node_id": node_id, "node_result_id": node_result_id, "state": state, "ts": ts, "data": data, "source_execution_id": source_execution_id, "input_values": input_values }.reject do | _k, v |
  v == OMIT
end
    end
