...
# A block that includes a combination of plain text and variable blocks.
  ...
  # @return [Object] 
    ...
    def initialize(state: OMIT, cache_config: OMIT, blocks:, additional_properties: nil)
      @state = state if state != OMIT
      @cache_config = cache_config if cache_config != OMIT
      @blocks = blocks
      @additional_properties = additional_properties
      @_field_set = { "state": state, "cache_config": cache_config, "blocks": blocks }.reject do | _k, v |
  v == OMIT
end
    end
